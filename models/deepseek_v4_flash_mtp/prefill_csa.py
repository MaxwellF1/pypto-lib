...
if __name__ == "__main__":
    ...
    if fixture_args.cp is None and "," in fixture_args.device:
...
CP_TMP_PAGES_DYN = pl.dynamic("CP_CSA_TMP_PAGES_DYN")
...
def owner_segments(cp_size: int) -> list[list[int]]:
    ...
    for segment in range(2 * cp_size):
        ...
        if table[rank][part] != -1:
            raise AssertionError(f"ownership collision at rank {rank}, part {part}")
...
def _build_block_tables(cp_size: int, candidate_history: int) -> dict[str, torch.Tensor]:
    ...
    # Compressed pools cover every row up to the request end.
    history_blocks = (candidate_history + CMP_STORAGE_BLOCK_SIZE - 1) // CMP_STORAGE_BLOCK_SIZE
    return {
        "cmp_block_table": table(PREFILL_CMP_MAX_BLOCKS, max(CP_PREFILL_CMP_BLOCK_NUM, history_blocks)),
        "idx_block_table": table(PREFILL_IDX_MAX_BLOCKS, max(PREFILL_IDX_BLOCK_NUM, history_blocks)),
        "compress_state_block_table": table(MAIN_STATE_MAX_BLOCKS, CSA_STATE_PHYSICAL_BLOCKS),
        "inner_compress_state_block_table": table(INNER_STATE_MAX_BLOCKS, CSA_INNER_STATE_PHYSICAL_BLOCKS),
    }
...
def _build_metadata_tensors(
    ...
    *, num_tokens: int | None = None, prefix: int = 0,
    ...
    tables = _build_block_tables(cp_size, candidate_history)
    boundary_positions = torch.full((nseg, MAX_COMPRESSED_ROWS_PER_SEGMENT), -1, dtype=torch.int32)
    ...
    seed_positions = torch.full((nseg, MAX_SEED_ROWS), -1, dtype=torch.int32)
    seed_lengths = torch.zeros(nseg, dtype=torch.int32)
    seed_cache_mapping = torch.full((cp_size, nseg, MAX_SEED_ROWS), -1, dtype=torch.int32)
    ...
    for segment in range(nseg):
        ...
        seed_positions[segment, :seed_len] = torch.arange(seed_start, starts[segment], dtype=torch.int32)
    ...
    for rank in range(cp_size):
        ...
        for segment in range(nseg):
            for row in range(MAX_COMPRESSED_ROWS_PER_SEGMENT):
                ...
                main_slot_mapping[rank, segment, row] = _lower_row(cmp_table, logical_main, CMP_STORAGE_BLOCK_SIZE)
                idx_slot_mapping[rank, segment, row] = _lower_row(idx_table, logical_idx, CMP_STORAGE_BLOCK_SIZE)

    segment_active_lengths = torch.zeros((cp_size, LOCAL_PARTS), dtype=torch.int32)
    tile_active_lengths = torch.full((cp_size, LOCAL_PARTS, MAX_SEGMENT_TILES), 0, dtype=torch.int32)
    predecessor_segments = torch.full((cp_size, LOCAL_PARTS), -1, dtype=torch.int32)
    query_position_ids = torch.full((cp_size, LOCAL_PARTS, MAX_SEGMENT_TILES, T), -1, dtype=torch.int32)
    ...
    final_segment = max((segment for segment, length in enumerate(lengths) if length), default=-1)
    ...
    final_main_state_mapping = torch.full((cp_size, STATE_LEN), -1, dtype=torch.int32)
    ...
    for rank in range(cp_size):
        ...
        for row, position in enumerate(final_snapshot_positions.tolist()):
            final_main_state_mapping[rank, row] = _lower_row(main_table, position, MAIN_STATE_BLOCK_SIZE)
            final_inner_state_mapping[rank, row] = _lower_row(inner_table, position, INNER_STATE_BLOCK_SIZE)

    def lower_prefix(table_name):
        prefix_rows = torch.full((cp_size, CP_CANDIDATE_CAPACITY), -1, dtype=torch.int32)
        slots = torch.arange(candidate_history, dtype=torch.int64)
        blocks = slots // CMP_STORAGE_BLOCK_SIZE
        for rank in range(cp_size):
            table = tables[table_name][rank].to(torch.int64)
            in_table = blocks < table.numel()
            physical = torch.where(in_table, table[blocks.clamp(max=table.numel() - 1)], torch.full_like(blocks, -1))
            rows = physical * CMP_STORAGE_BLOCK_SIZE + slots % CMP_STORAGE_BLOCK_SIZE
            prefix_rows[rank, :candidate_history] = torch.where(physical >= 0, rows, torch.full_like(rows, -1)).to(torch.int32)
        return prefix_rows

    dense_idx_prefix = lower_prefix("idx_block_table")
    dense_cmp_prefix = lower_prefix("cmp_block_table")
...
def _build_raw_attention_metadata(cp_size: int, *, num_tokens: int | None = None, prefix: int = 0) -> tuple[dict[str, torch.Tensor], dict[str, object]]:
    ...
    overlay_positions = torch.full((cp_size, LOCAL_PARTS, MAX_SEGMENT_TILES, OVERLAY_ROWS), -1, dtype=torch.int32)
        ...
        for part in range(LOCAL_PARTS):
            ...
            for tile in range(MAX_SEGMENT_TILES):
                ...
                if predecessor_len:
                    ...
                    ] = torch.arange(predecessor_start, predecessor_start + predecessor_len, dtype=torch.int32)
                    overlay_requests[rank, part, tile, :predecessor_len] = 0
                if active:
                    overlay_positions[
                        rank, part, tile, T : T + active
                    ] = torch.arange(tile_start, tile_start + active, dtype=torch.int32)
...
def build_csa_leaf_metadata(cp_size, metadata, raw, raw_ctx):
    ...
    leaf_num_tokens_input = torch.zeros(cp_size, LOCAL_PARTS, MAX_COMPRESS_LEAVES, dtype=torch.int32)
...
def build_cp_tensor_specs(cp_size: int = CP_SIZE, *, num_tokens: int | None = None, prefix: int = 0):
    ...
    for rank in range(cp_size):
        ...
        for position, value in logical_main_state.items():
            row = _lower_row(metadata["compress_state_block_table"][rank], position, MAIN_STATE_BLOCK_SIZE)
            if row >= 0:
                main_flat[row] = value
        for position, value in logical_inner_state.items():
            row = _lower_row(metadata["inner_compress_state_block_table"][rank], position, INNER_STATE_BLOCK_SIZE)
            if row >= 0:
                inner_flat[row] = value

    cmp_blocks = int(metadata["cmp_block_table"].max()) + 1
    idx_blocks = int(metadata["idx_block_table"].max()) + 1
    cmp_cache = torch.zeros(cp_size, cmp_blocks, CMP_STORAGE_BLOCK_SIZE, 1, HEAD_DIM, dtype=torch.bfloat16)
    idx_cache = torch.zeros(cp_size, idx_blocks, CMP_STORAGE_BLOCK_SIZE, 1, IDX_HEAD_DIM, dtype=torch.int8)
    idx_scale = torch.zeros(cp_size, idx_blocks, CMP_STORAGE_BLOCK_SIZE, 1, 1, dtype=torch.float32)
    ...
    for rank in range(cp_size):
        ...
        for logical_slot in range(completed):
            cmp_row = _lower_row(metadata["cmp_block_table"][rank], logical_slot, CMP_STORAGE_BLOCK_SIZE)
            idx_row = _lower_row(metadata["idx_block_table"][rank], logical_slot, CMP_STORAGE_BLOCK_SIZE)
            if cmp_row >= 0:
...
def _cp_csa_compress_pack_part(
    ...
    main_state_workspace: pl.Tensor[[MAIN_STATE_BLOCKS_DYN, MAIN_STATE_BLOCK_SIZE, MAIN_STATE_DIM], pl.FP32],
    inner_state_workspace: pl.Tensor[[INNER_STATE_BLOCKS_DYN, INNER_STATE_BLOCK_SIZE, INNER_STATE_DIM], pl.FP32],
    ...
    main_cache = pl.create_tensor([CP_PREFILL_CMP_BLOCK_NUM, CMP_STORAGE_BLOCK_SIZE, 1, HEAD_DIM], dtype=pl.BF16)
    idx_cache = pl.create_tensor([idx_cache_blocks, CMP_STORAGE_BLOCK_SIZE, 1, IDX_HEAD_DIM], dtype=pl.INT8)
    idx_scale = pl.create_tensor([idx_cache_blocks, CMP_STORAGE_BLOCK_SIZE, 1, 1], dtype=pl.FP32)
    ...
    with pl.at(level=pl.Level.CORE_GROUP, name_hint="cp_csa_seed_part_metadata") as part_meta_seed_tid:
        record_meta[:, :] = pl.full([EPOCHS * MAX_COMPRESSED_ROWS_PER_SEGMENT, META_DIM], dtype=pl.INT32, value=-1)
        main_state_meta[:, :] = pl.full([EPOCHS * STATE_ROWS_PER_RANK, STATE_META_DIM], dtype=pl.INT32, value=-1)
        inner_state_meta[:, :] = pl.full([EPOCHS * STATE_ROWS_PER_RANK, STATE_META_DIM], dtype=pl.INT32, value=-1)
                ...
                for row in pl.range(T):
                    if row < active:
                        ...
                        if main_source_i64 >= 0 and idx_source_i64 >= 0:
                            logical_slot = pl.cast((position + 1) // COMPRESS_RATIO - 1, pl.INT32)
                            ...
                            for epoch in pl.range(EPOCHS):
                                ...
                                for scale_col in pl.range(SCALE_TILE_COLS):
                                    pl.write(idx_scale_payload, [destination, scale_col], scale_value)
                                pl.write(record_meta, [destination, 0], pl.cast(1, pl.INT32))
                                pl.write(record_meta, [destination, 1], segment)
                                pl.write(record_meta, [destination, 2], position)
                                pl.write(record_meta, [destination, 3], pl.cast(record_row, pl.INT32))
                                pl.write(record_meta, [destination, 4], pl.cast(1, pl.INT32))
            ...
            for state_row in pl.range(STATE_ROWS_PER_RANK):
                ...
                if position < segment_end and main_block >= 0 and inner_block >= 0:
                    ...
                    for epoch in pl.range(EPOCHS):
                        ...
                        pl.write(main_state_meta, [destination, 0], pl.cast(1, pl.INT32))
                        pl.write(main_state_meta, [destination, 1], position)
                        pl.write(main_state_meta, [destination, 2], position)
                        pl.write(inner_state_meta, [destination, 0], pl.cast(1, pl.INT32))
...
def _prefill_cp_csa_history_exchange(
    ...
    cmp_root: pl.InOut[pl.Tensor[[CP_TMP_PAGES_DYN, BLOCK_SIZE, 1, HEAD_DIM], pl.BF16]],
    idx_root: pl.InOut[pl.Tensor[[CP_TMP_PAGES_DYN, BLOCK_SIZE, 1, IDX_HEAD_DIM], pl.INT8]],
    scale_root: pl.InOut[pl.Tensor[[CP_TMP_PAGES_DYN, BLOCK_SIZE, 1, 1], pl.FP16]],
    ...
    root_rows = pl.tensor.dim(cmp_root, 0) * BLOCK_SIZE
    cmp_dst = pl.reshape(cmp_root, [root_rows, HEAD_DIM])
    idx_dst = pl.reshape(idx_root, [root_rows, IDX_HEAD_DIM])
    scale_dst = pl.reshape(scale_root, [root_rows, 1])
...
def prefill_attention_csa(
    ...
    inner_ape: pl.Tensor[[COMPRESS_RATIO, INNER_OUT_DIM], pl.FP32],
    inner_norm_w: pl.Tensor[[IDX_HEAD_DIM], pl.BF16],
    main_state_workspace0: pl.Tensor[[MAIN_STATE_BLOCKS_DYN, MAIN_STATE_BLOCK_SIZE, MAIN_STATE_DIM], pl.FP32],
    inner_state_workspace0: pl.Tensor[[INNER_STATE_BLOCKS_DYN, INNER_STATE_BLOCK_SIZE, INNER_STATE_DIM], pl.FP32],
    main_state_workspace1: pl.Tensor[[MAIN_STATE_BLOCKS_DYN, MAIN_STATE_BLOCK_SIZE, MAIN_STATE_DIM], pl.FP32],
    inner_state_workspace1: pl.Tensor[[INNER_STATE_BLOCKS_DYN, INNER_STATE_BLOCK_SIZE, INNER_STATE_DIM], pl.FP32],
    compress_state: pl.InOut[pl.Tensor[[MAIN_STATE_BLOCKS_DYN, MAIN_STATE_BLOCK_SIZE, MAIN_STATE_DIM], pl.FP32]],
    compress_state_block_table: pl.Tensor[[MAIN_STATE_MAX_BLOCKS], pl.INT32],
    inner_compress_state: pl.InOut[pl.Tensor[[INNER_STATE_BLOCKS_DYN, INNER_STATE_BLOCK_SIZE, INNER_STATE_DIM], pl.FP32]],
    inner_compress_state_block_table: pl.Tensor[[INNER_STATE_MAX_BLOCKS], pl.INT32],
    kv_cache: pl.InOut[pl.Tensor[[RAW_BLOCKS_DYN, BLOCK_SIZE, 1, HEAD_DIM], pl.BF16]],
    cmp_kv: pl.InOut[pl.Tensor[[CP_CMP_BLOCK_NUM_DYN, CMP_STORAGE_BLOCK_SIZE, 1, HEAD_DIM], pl.BF16]],
    cmp_block_table: pl.Tensor[[PREFILL_CMP_MAX_BLOCKS], pl.INT32],
    idx_kv_cache: pl.InOut[pl.Tensor[[IDX_BLOCKS_DYN, CMP_STORAGE_BLOCK_SIZE, 1, IDX_HEAD_DIM], pl.INT8]],
    idx_kv_scale: pl.InOut[pl.Tensor[[IDX_BLOCKS_DYN, CMP_STORAGE_BLOCK_SIZE, 1, 1], pl.FP32]],
    idx_block_table: pl.Tensor[[IDX_CACHE_MAX_BLOCKS], pl.INT32],
    ...
    query_positions: pl.Tensor[[LOCAL_PARTS, MAX_SEGMENT_TILES, T], pl.INT32],
    ...
    leaf_main_state_slots_input: pl.Tensor[[LOCAL_PARTS, MAX_COMPRESS_LEAVES, T], pl.INT64],
    leaf_inner_state_slots_input: pl.Tensor[[LOCAL_PARTS, MAX_COMPRESS_LEAVES, T], pl.INT64],
    leaf_num_tokens_input: pl.Tensor[[LOCAL_PARTS, MAX_COMPRESS_LEAVES], pl.INT32],
    ...
    completion_token: pl.Out[pl.Tensor[[NUM_MOE_WAVES, 1, 8], pl.FP32]],
    cache_owner_rank: pl.Scalar[pl.INT32],
    my_rank: pl.Scalar[pl.INT32],
    ...
    cp_tmp_candidate_rows = pl.min(history_base // COMPRESS_RATIO + CP_TMP_COMPRESSED_ROWS, CP_CANDIDATE_CAPACITY)
    cp_tmp_cache_pages = 1 + (cp_tmp_candidate_rows + BLOCK_SIZE - 1) // BLOCK_SIZE
    cp_tmp_cache_rows = cp_tmp_cache_pages * BLOCK_SIZE
    cp_tmp_scale_groups = cp_tmp_cache_rows // 16
    ...
    cp_tmp_idx_scale_aligned = pl.reshape(cp_tmp_idx_scale, [cp_tmp_scale_groups, 16])
    ...
    with pl.at(level=pl.Level.CORE_GROUP, name_hint="cp_csa_tmp_root_seed") as cp_tmp_seed_tid:
        ...
        for logical_page in pl.range(cp_tmp_cache_pages - 1):
    ...
    for tile in pl.range(NUM_LOCAL_TILES):
        ...
        materialize_rope_rows(freqs_cos, freqs_sin, position_tile, active, cos_tile, sin_tile)
    ...
    with pl.at(level=pl.Level.CORE_GROUP, name_hint="cp_csa_remote_hidden_lowering", deps=[tail_exchange_tid]) as remote_hidden_tid:
        for part in pl.range(LOCAL_PARTS):
    ...
    scratch_inner1_flat = pl.reshape(inner_state_workspace1, [inner_state_rows, INNER_STATE_DIM])
    ...
    part0_main_state_slots = pl.slice(leaf_main_state_slots, [part_leaf_rows], [0])
    part0_inner_state_slots = pl.slice(leaf_inner_state_slots, [part_leaf_rows], [0])
    part0_num_tokens = pl.slice(leaf_num_tokens_flat, [MAX_COMPRESS_LEAVES], [0])
    part0_segment = pl.read(owner_segments_t, [0])
    part0_segment_start = pl.read(segment_starts_local, [pl.cast(part0_segment, pl.INDEX)])
    (
        part0_main_payload,
        part0_idx_payload,
        ...
        idx_block_table, idx_scratch_blocks,
        cmp_wkv, cmp_wgate, cmp_ape, cmp_norm_w,
        inner_wkv, inner_wgate, inner_ape, inner_norm_w,
        freqs_cos, freqs_sin,
        hadamard_idx,
    ...
    part1_main_slots = pl.slice(leaf_main_slots, [part_leaf_rows], [part1_row0])
    part1_idx_slots = pl.slice(leaf_idx_slots, [part_leaf_rows], [part1_row0])
    part1_main_state_slots = pl.slice(leaf_main_state_slots, [part_leaf_rows], [part1_row0])
    part1_inner_state_slots = pl.slice(leaf_inner_state_slots, [part_leaf_rows], [part1_row0])
    part1_num_tokens = pl.slice(leaf_num_tokens_flat, [MAX_COMPRESS_LEAVES], [MAX_COMPRESS_LEAVES])
    part1_segment = pl.read(owner_segments_t, [1])
    part1_segment_start = pl.read(segment_starts_local, [pl.cast(part1_segment, pl.INDEX)])
    (
        part1_main_payload,
        part1_idx_payload,
    ...
    packed_main_payload = pl.create_tensor([EPOCHS * ROWS_PER_RANK, HEAD_DIM], dtype=pl.BF16)
    packed_idx_payload = pl.create_tensor([EPOCHS * ROWS_PER_RANK, IDX_HEAD_DIM], dtype=pl.INT8)
    packed_idx_scale_payload = pl.create_tensor([EPOCHS * ROWS_PER_RANK, SCALE_TILE_COLS], dtype=pl.FP16)
    packed_record_meta = pl.create_tensor([EPOCHS * ROWS_PER_RANK, META_DIM], dtype=pl.INT32)
    packed_main_state_payload = pl.create_tensor([EPOCHS * STATE_ROWS_PER_RANK, MAIN_STATE_DIM], dtype=pl.FP32)
    packed_inner_state_payload = pl.create_tensor([EPOCHS * STATE_ROWS_PER_RANK, INNER_STATE_DIM], dtype=pl.FP32)
    packed_main_state_meta = pl.create_tensor([EPOCHS * STATE_ROWS_PER_RANK, STATE_META_DIM], dtype=pl.INT32)
    packed_inner_state_meta = pl.create_tensor([EPOCHS * STATE_ROWS_PER_RANK, STATE_META_DIM], dtype=pl.INT32)

    with pl.at(level=pl.Level.CORE_GROUP, name_hint="cp_csa_seed_packed_state_metadata") as packed_state_meta_seed_tid:
        packed_main_state_meta[:, :] = pl.full([EPOCHS * STATE_ROWS_PER_RANK, STATE_META_DIM], dtype=pl.INT32, value=-1)
                    ...
                    if record_valid > 0 and logical_segment >= 0 and boundary >= 0:
                        ...
                        if (idx_valid > 0 and idx_slot >= 0 and idx_slot < cp_tmp_candidate_rows):
                            ...
                            if logical_block < IDX_CACHE_MAX_BLOCKS:
                                physical_block = pl.read(idx_block_table, [logical_block])
                ...
                for state_row in pl.range(STATE_ROWS_PER_RANK):
                    ...
                    if inner_valid > 0 and inner_position >= 0:
                        ...
                        if logical_block < INNER_STATE_MAX_BLOCKS:
                            physical_block = pl.read(inner_compress_state_block_table, [logical_block])
                            if my_rank == cache_owner_rank and physical_block >= 0:
                                destination = (
                                    pl.cast(physical_block, pl.INDEX)
                                    * INNER_STATE_BLOCK_SIZE
    ...
    with pl.spmd(LOCAL_ROWS // CSA_TOPK_SEED_ROWS, name_hint="cp_csa_seed_topk_indices") as cmp_indices_seed_tid:
        seed_block = pl.tile.get_block_idx()
        seed_row0 = seed_block * CSA_TOPK_SEED_ROWS
        cmp_indices[
            seed_row0 : seed_row0 + CSA_TOPK_SEED_ROWS, 0:IDX_TOPK
        ] = pl.full([CSA_TOPK_SEED_ROWS, IDX_TOPK], dtype=pl.INT32, value=-1)
    ...
    for tile in pl.range(NUM_LOCAL_TILES):
        ...
        if active > 0:
            ...
            with pl.at(level=pl.Level.CORE_GROUP, name_hint="cp_csa_indexer_rope_rows"):
                idx_cos[0:T, :] = pl.cast(rope_cos_flat[row0 : row0 + T, 0:QK_ROPE_HALF], target_type=pl.FP32)
                idx_sin[0:T, :] = pl.cast(rope_sin_flat[row0 : row0 + T, 0:QK_ROPE_HALF], target_type=pl.FP32)
    ...
    with pl.spmd(NUM_LOCAL_TILES, name_hint="cp_csa_raw_current_scatter", deps=[raw_seed_tid]) as raw_current_tid:
    ...
    with pl.spmd(LOCAL_PARTS, name_hint="cp_csa_raw_predecessor_scatter", deps=[raw_current_tid]) as raw_predecessor_tid:
    ...
    with pl.at(level=pl.Level.CORE_GROUP, name_hint="cp_csa_rank_complete", deps=[resource_done_tid]):
...
def prefill_cp_csa_rank(
    ...
    # passed into prefill_attention_csa (which publishes it via its terminal
    ...
    completion_token = pl.create_tensor([NUM_MOE_WAVES, 1, 8], dtype=pl.FP32)
    return prefill_attention_csa(
...
def prefill_cp_csa_test(
    x_hc: pl.Tensor[[CP_SIZE, LOCAL_PARTS, MAX_SEGMENT_TILES, T, HC_MULT, D], pl.FP32],
    hc_attn_fn: pl.Tensor[[MIX_HC, HC_DIM], pl.FP32],
    hc_attn_scale: pl.Tensor[[3], pl.FP32],
    hc_attn_base: pl.Tensor[[MIX_HC], pl.FP32],
    attn_norm_w: pl.Tensor[[D], pl.BF16],
    ...
    inner_compress_state: pl.InOut[pl.Tensor[[CP_SIZE, INNER_STATE_BLOCKS_DYN, INNER_STATE_BLOCK_SIZE, INNER_STATE_DIM], pl.FP32]],
    inner_compress_state_block_table: pl.Tensor[[CP_SIZE, INNER_STATE_MAX_BLOCKS], pl.INT32],
    kv_cache: pl.InOut[pl.Tensor[[CP_SIZE, RAW_BLOCKS_DYN, BLOCK_SIZE, 1, HEAD_DIM], pl.BF16]],
    cmp_kv: pl.InOut[pl.Tensor[[CP_SIZE, CP_CMP_BLOCK_NUM_DYN, CMP_STORAGE_BLOCK_SIZE, 1, HEAD_DIM], pl.BF16]],
    cmp_block_table: pl.Tensor[[CP_SIZE, PREFILL_CMP_MAX_BLOCKS], pl.INT32],
    idx_kv_cache: pl.InOut[pl.Tensor[[CP_SIZE, IDX_BLOCKS_DYN, CMP_STORAGE_BLOCK_SIZE, 1, IDX_HEAD_DIM], pl.INT8]],
    idx_kv_scale: pl.InOut[pl.Tensor[[CP_SIZE, IDX_BLOCKS_DYN, CMP_STORAGE_BLOCK_SIZE, 1, 1], pl.FP32]],
    ...
    query_positions: pl.Tensor[[CP_SIZE, LOCAL_PARTS, MAX_SEGMENT_TILES, T], pl.INT32],
    query_requests: pl.Tensor[[CP_SIZE, LOCAL_PARTS, MAX_SEGMENT_TILES, T], pl.INT32],
    overlay_positions: pl.Tensor[[CP_SIZE, LOCAL_PARTS, MAX_SEGMENT_TILES, OVERLAY_ROWS], pl.INT32],
    ...
    leaf_main_state_slots_input: pl.Tensor[[CP_SIZE, LOCAL_PARTS, MAX_COMPRESS_LEAVES, T], pl.INT64],
    leaf_inner_state_slots_input: pl.Tensor[[CP_SIZE, LOCAL_PARTS, MAX_COMPRESS_LEAVES, T], pl.INT64],
    leaf_num_tokens_input: pl.Tensor[[CP_SIZE, LOCAL_PARTS, MAX_COMPRESS_LEAVES], pl.INT32],
    main_state_workspace0: pl.Tensor[[CP_SIZE, MAIN_STATE_BLOCKS_DYN, MAIN_STATE_BLOCK_SIZE, MAIN_STATE_DIM], pl.FP32],
    inner_state_workspace0: pl.Tensor[[CP_SIZE, INNER_STATE_BLOCKS_DYN, INNER_STATE_BLOCK_SIZE, INNER_STATE_DIM], pl.FP32],
    main_state_workspace1: pl.Tensor[[CP_SIZE, MAIN_STATE_BLOCKS_DYN, MAIN_STATE_BLOCK_SIZE, MAIN_STATE_DIM], pl.FP32],
    inner_state_workspace1: pl.Tensor[[CP_SIZE, INNER_STATE_BLOCKS_DYN, INNER_STATE_BLOCK_SIZE, INNER_STATE_DIM], pl.FP32],
    x_out: pl.Out[pl.Tensor[[CP_SIZE, LOCAL_PARTS, MAX_SEGMENT_TILES, T, HC_MULT, D], pl.FP32]],
):
    hidden_tail_buf = pld.alloc_window_buffer([CP_TAIL_WINDOW_ROWS, D], dtype=pl.BF16)
    ...
    main_buf = pld.alloc_window_buffer([RECORDS_PER_WINDOW, HEAD_DIM], dtype=pl.BF16)
    idx_buf = pld.alloc_window_buffer([RECORDS_PER_WINDOW, IDX_HEAD_DIM], dtype=pl.INT8)
    scale_buf = pld.alloc_window_buffer([RECORDS_PER_WINDOW, SCALE_TILE_COLS], dtype=pl.FP16)
    record_buf = pld.alloc_window_buffer([RECORDS_PER_WINDOW, META_DIM], dtype=pl.INT32)
    main_state_buf = pld.alloc_window_buffer([STATE_RECORDS_PER_WINDOW, MAIN_STATE_DIM], dtype=pl.FP32)
    main_state_meta_buf = pld.alloc_window_buffer([STATE_RECORDS_PER_WINDOW, STATE_META_DIM], dtype=pl.INT32)
    inner_state_buf = pld.alloc_window_buffer([STATE_RECORDS_PER_WINDOW, INNER_STATE_DIM], dtype=pl.FP32)
    inner_state_meta_buf = pld.alloc_window_buffer([STATE_RECORDS_PER_WINDOW, STATE_META_DIM], dtype=pl.INT32)
    ...
    for rank in pl.range(pld.world_size()):
        hidden_tail_window = pld.window(hidden_tail_buf, [CP_TAIL_WINDOW_ROWS, D], dtype=pl.BF16)
        tail_ready = pld.window(tail_ready_buf, [CP_SIZE, 1], dtype=pl.INT32)
        tail_consumed = pld.window(tail_consumed_buf, [CP_SIZE, 1], dtype=pl.INT32)
        main_window = pld.window(main_buf, [RECORDS_PER_WINDOW, HEAD_DIM], dtype=pl.BF16)
        idx_window = pld.window(idx_buf, [RECORDS_PER_WINDOW, IDX_HEAD_DIM], dtype=pl.INT8)
        scale_window = pld.window(scale_buf, [RECORDS_PER_WINDOW, SCALE_TILE_COLS], dtype=pl.FP16)
        record_window = pld.window(record_buf, [RECORDS_PER_WINDOW, META_DIM], dtype=pl.INT32)
        main_state_window = pld.window(main_state_buf, [STATE_RECORDS_PER_WINDOW, MAIN_STATE_DIM], dtype=pl.FP32)
        main_state_meta_window = pld.window(
            main_state_meta_buf,
            [STATE_RECORDS_PER_WINDOW, STATE_META_DIM],
            dtype=pl.INT32,
        )
        inner_state_window = pld.window(inner_state_buf, [STATE_RECORDS_PER_WINDOW, INNER_STATE_DIM], dtype=pl.FP32)
        ...
        prefill_cp_csa_rank(
            ...
            segment_starts_t, segment_lengths_t, segment_active_lengths[rank],
...
def golden_prefill_cp_csa(tensors):
    ...
    local_kv = torch.zeros(cp_size, LOCAL_PARTS, MAX_SEGMENT_TILES, T, HEAD_DIM, dtype=torch.bfloat16)
    local_qr = torch.zeros(cp_size, LOCAL_PARTS, MAX_SEGMENT_TILES, T, Q_LORA, dtype=torch.int8)
    local_qr_scale = torch.zeros(cp_size, LOCAL_PARTS, MAX_SEGMENT_TILES, T, 1, dtype=torch.float32)
    local_post = torch.zeros(cp_size, LOCAL_PARTS, MAX_SEGMENT_TILES, T, HC_MULT)
    local_comb = torch.zeros(cp_size, LOCAL_PARTS, MAX_SEGMENT_TILES, T, HC_MULT * HC_MULT)
                ...
                golden_qkv_proj_rope(
                    {
                        ...
                        "wkv": tensors["wkv"],
                        "rope_cos": tensors["freqs_cos"].index_select(0, rope_rows),
                        "rope_sin": tensors["freqs_sin"].index_select(0, rope_rows),
    ...
    for segment in range(NUM_SEGMENTS):
        ...
        if segment > 0 and lengths[segment] > 0:
            ...
            seed_len = min(starts[segment] % COMPRESS_RATIO + COMPRESS_RATIO, min(T, lengths[predecessor]))
            ...
            if seed_len:
                ...
                positions[:seed_len] = torch.arange(starts[segment] - seed_len, starts[segment], dtype=torch.int32)
            leaves.append((seed, positions))
        else:
            leaves.append((torch.zeros(T, D, dtype=torch.bfloat16), torch.zeros(T, dtype=torch.int32)))
        for tile in range(MAX_SEGMENT_TILES):
            leaves.append((local_norm[rank, part, tile], tensors["query_positions"][rank, part, tile]))

        for leaf, (x_leaf, positions) in enumerate(leaves):
            if leaf == 0:
                active = (
                    min(starts[segment] % COMPRESS_RATIO + COMPRESS_RATIO, min(T, lengths[segment - 1]))
                    if segment > 0 and lengths[segment] > 0
                    else 0
                )
            else:
            ...
            if active <= 0:
                continue
            main_cache = torch.zeros(MAIN_LEAF_CACHE_BLOCKS, CMP_STORAGE_BLOCK_SIZE, 1, HEAD_DIM, dtype=torch.bfloat16)
            idx_cache = torch.zeros(IDX_LEAF_CACHE_BLOCKS, CMP_STORAGE_BLOCK_SIZE, 1, IDX_HEAD_DIM, dtype=torch.int8)
            idx_scale = torch.zeros(IDX_LEAF_CACHE_BLOCKS, CMP_STORAGE_BLOCK_SIZE, 1, 1)
            ...
            golden_prefill_indexer_compressor(
                {
                    ...
                    "kv": torch.zeros(T // COMPRESS_RATIO, IDX_HEAD_DIM, dtype=torch.int8),
                    "idx_kv_cache": idx_cache,
                    "idx_kv_scale": idx_scale,
    ...
    for rank in range(cp_size):
        ...
        for logical_slot, value in cmp_published.items():
            row = _lower_row(tensors["cmp_block_table"][rank], logical_slot, CMP_STORAGE_BLOCK_SIZE)
            if row >= 0:
                cmp_flat[row] = value
        for logical_slot, value in idx_published.items():
            row = _lower_row(tensors["idx_block_table"][rank], logical_slot, CMP_STORAGE_BLOCK_SIZE)
            if row >= 0:
                idx_flat[row] = value
    ...
    for receiver in range(cp_size):
        for position in final_positions:
            ...
            destination = _lower_row(tensors["compress_state_block_table"][receiver], position, MAIN_STATE_BLOCK_SIZE)
            if source >= 0 and destination >= 0:
    ...
    for rank in range(cp_size):
        ...
        rope_rows = positions.clamp_min(0).long()
        cos = tensors["freqs_cos"].index_select(0, rope_rows)[:, :QK_ROPE_HALF].float().unsqueeze(1)
        sin = tensors["freqs_sin"].index_select(0, rope_rows)[:, :QK_ROPE_HALF].float().unsqueeze(1)
        query = torch.cat(
            [
                query[..., :-ROPE_HEAD_DIM],
                torch.stack(
        ...
        max_visible = min(CP_CANDIDATE_CAPACITY, max(0, (int(positions.max()) + 1) // COMPRESS_RATIO))
        cache_rows = []
        cache_scales = []
        idx_flat = idx_out[rank].view(-1, IDX_HEAD_DIM)
        ...
        for logical_slot in range(max_visible):
            row = _lower_row(tensors["idx_block_table"][rank], logical_slot, CMP_STORAGE_BLOCK_SIZE)
            cache_rows.append(idx_flat[row] if row >= 0 else torch.zeros(IDX_HEAD_DIM, dtype=torch.int8))
        ...
        if max_visible:
            ...
            score = torch.empty(LOCAL_ROWS, max_visible, dtype=torch.float32)
            for row0 in range(0, LOCAL_ROWS, GOLDEN_SCORE_ROW_CHUNK):
                row1 = min(row0 + GOLDEN_SCORE_ROW_CHUNK, LOCAL_ROWS)
                for col0 in range(0, max_visible, GOLDEN_SCORE_COL_CHUNK):
                    col1 = min(col0 + GOLDEN_SCORE_COL_CHUNK, max_visible)
                    chunk = torch.einsum("thd,cd->thc", query_i32[row0:row1], cache_i8[col0:col1]).float()
                    chunk = chunk * query_scale[row0:row1] * cache_scale[:, :, col0:col1]
                    score[row0:row1, col0:col1] = (torch.relu(chunk) * weights[row0:row1].unsqueeze(-1)).sum(dim=1)
            for row in range(LOCAL_ROWS):
                ...
                if selected:
                    topk[row, :selected] = score[row, :visible].topk(selected).indices.to(torch.int32)
        topk_by_rank.append(topk)

    kv_before_commit = tensors["kv_cache"].clone()
    x_out = torch.zeros_like(tensors["x_out"])
    for rank in range(cp_size):
        topk = topk_by_rank[rank].view(LOCAL_PARTS, MAX_SEGMENT_TILES, T, IDX_TOPK)
        for part in range(LOCAL_PARTS):
            segment = int(owners[rank][part])
            for tile in range(MAX_SEGMENT_TILES):
                active = _active_tile(lengths[segment], tile)
                overlay = torch.zeros(OVERLAY_ROWS, HEAD_DIM, dtype=torch.bfloat16)
                predecessor_len = int(tensors["overlay_active_lengths"][rank, part, tile, 0])
                current_len = int(tensors["overlay_active_lengths"][rank, part, tile, 1])
                if predecessor_len:
                    if tile == 0:
                        predecessor = segment - 1
                ...
                golden_prefill_sparse_attn(
                    {
                        ...
                        "num_tokens": active,
                        "freqs_cos": tensors["freqs_cos"].index_select(0, positions.clamp_min(0).long()),
                        "freqs_sin": tensors["freqs_sin"].index_select(0, positions.clamp_min(0).long()),
                        "wo_a": tensors["wo_a"],
                        "wo_b": tensors["wo_b"],
